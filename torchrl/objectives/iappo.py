...
class IAPPOLoss(LossModule):
    """A parent Intervention-Assisted (IA) PPO loss class.
    ...
    class _AcceptedKeys:
        ...
        Attributes:
            ...
            action (NestedKey): The input tensordict key where the action is expected.
                Defaults to ``"action"``.
            intervention (NestedKey): The input tensordict key where the intervention
                indicator is expected. Defaults to ``"intervention"``.
        ...
        intervention: NestedKey = "intervention"
...
class ClipIAPPOLoss(IAPPOLoss):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(ClipIAPPOLoss, self).__init__(
            actor_network,
            critic_network,
    ...
    def _get_onpolicy_data(self, tensordict: TensorDictBase) -> TensorDictBase:
        onpolicy_indices,_ = torch.where(tensordict.get("intervene")==False)
        tensordict = tensordict[onpolicy_indices]
        return tensordict
    @dispatch
    def forward(self, tensordict: TensorDictBase) -> TensorDictBase:
        # create new tensordict using only indices where intervene is true
        tensordict = self._get_onpolicy_data(tensordict)
        tensordict = tensordict.clone(False)
